...
#         if row[columns[1]] <= delta and row[columns[1]] >= -1 * delta:
#             if not run_going:
#                 run_start = index
#             run_going = True
#
...
#             if run_going:
#                 run_end = index
#                 avg = ((run_end - run_start) // 2) + run_start
#                 zeroes.append(data_frame.iloc[int(avg)])
#
#             run_going = False
#
...
#     Finds the peaks in the given data_frame!
#
#     Adds to the 4th row whether this is a maximum (0) or minimum (1) value
#     :param data_frame:
#     :return:
#     """
...
#
#         if row[2] <= delta and row[2] >= -1 * delta:
#             if not run_going:
#                 run_start = index
#             run_going = True
#
...
#                 avg = ((run_end - run_start) // 2) + run_start
#
#                 if index + context < data_frame.shape[0]:
#                     if data_frame[index + context][2] > row[2]:
#                         # local min
#                         pos = True
#                     else:
#                         pos = False
#                 elif index + context > data_frame.shape[0]:
#                     if data_frame[index - context][2] > row[2]:
#                         # local min
#                         pos = True
#                     else:
#                         pos = False
#                 else:
#                     print(f'error! Sample size too small for context. Try lowering context.')
#
#
#                 # if row[2] > 0:
#                 #     pos = True
...
#             run_going = False
#
#     zero_values = np.c_[data_frame[zeroes], types]
#
#     # df = pd.DataFrame([zeroes])
#
#     flip = zero_values[0, 3]
#     to_delete = []
#     for index, row in enumerate(zero_values):
#         # if flip != row[3]:
...
#
#         if index != 0:
#             dist = zero_values[index, 0] - zero_values[index - 1, 0]
#             if dist < min_dist:
#                 # print(f'FOUND BAD DISTANCE!')
#                 # print(f'row: {zero_values[index - 1]}')
#                 # print(f'row: {zero_values[index]}')
#                 # print(f'')
#                 to_delete.append(index - 1)
#
#     zero_values = np.delete(zero_values, to_delete, axis=0)
...
#
#     if data_frame[2, 2] >= 0:
#         print(f'start with increasing')
#         increasing = True
#     else:
#         print(f'start with decreasing')
#         increasing = False
#
#     print(f'dataframe size: {data_frame.shape}')
#
#
#     index = 0
#     while index < data_frame.shape[0]:
#
#         curr_value = data_frame[index, 1]
#         context_index = index + context
#         if index + context < len(data_frame):
#             context_value = data_frame[context_index, 1]
#         else:
#             context_value = None
#
#         if increasing:
#             if context_value and context_value < curr_value:
#                 print(f'local max found!')
#                 print(f'{data_frame[index:context_index]}')
#                 print(f'max: {np.argmax(data_frame[index:context_index], axis=0)}')
#                 max_index = np.argmax(data_frame[index:context_index], axis=0)
#                 max_value = data_frame[max_index]
